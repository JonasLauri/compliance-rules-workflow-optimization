...
Sub AutoRulesDocumentation()
    ...
    '4. Adds an indicator which week and when rules were implemented
    wsDest.Cells(wsDest.UsedRange.Rows.Count, "A") _
      .Offset(1).Value = "Week" & numWeek & " " & "(" & Date & ")"
    
    '5. Copy & Paste Data
    wsCopy.Range("B2:N" & lCopyLastRow).Copy _
      wsDest.Range("B" & lDestLastRow)
      
    '6. Closes new rules workbook
    Workbooks("Rules.xlsx").Close SaveChanges:=False
    
    'Update 2022-11-08: Additional funtionalities was added, reference to 1.2 _
    'Reference to 4
End Sub
